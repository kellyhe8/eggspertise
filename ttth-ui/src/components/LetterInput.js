    ...
    return (
      <div className="letterinput" >
        <ButtonGroup disableElevation  color="secondary" variant="contained" size="large" aria-label="outlined button group">
          {row1.map((l,_) => {
            return <Button key={l} disabled={this.props.guesses.includes(l) || !this.props.hintAnswers.includes(l) || this.props.won || (l !== this.props.answer && this.props.showAnswer)} onClick={() => this.props.checkGuess(l)}>{l}</Button>
          })}
        </ButtonGroup>
        <ButtonGroup disableElevation  color="secondary" variant="contained" size="large" aria-label="outlined button group">
          {row2.map((l,_) => {
